...
角色窗口支持置顶、鼠标穿透、系统级鼠标跟随、本机摄像头面捕、麦克风口型，以及当前项目真实具有的表情和动作。摄像头先记录自然睁眼和闭口基线，过滤非零中立分数、单眼误检与重连残留，避免眼睛或嘴型长期停在半透明交叉淡化状态。

视频录制可以选择透明、黑、白、绿幕或自定义纯色背景，设置输出分辨率、24/30/60 FPS、手动或定时停止，并可录入已经开启的麦克风。普通录制只生成 WebM；需要复现或调试时可额外勾选动作数据，保存鼠标跟随、面捕、口型、表情、动作和外部控制的可回放 JSON。回放会校验项目 revision 并暂时隔离实时输入。窗口意外关闭时保留 `.partial.webm` 和中断报告，不静默丢失录制。
