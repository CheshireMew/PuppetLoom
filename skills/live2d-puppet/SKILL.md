...
用户要求创建完整角色时，缺少左右闭眼、微张嘴或张口属于正常制作步骤，外部 Agent 默认生成、检查并接入真正缺少的素材，不再逐项索取授权。已有闭嘴就是 `mouthOpen=0`，不得重画。用户明确说“只使用现有素材”“不要生图”时才停止补素材并报告 `needs-assets`。闭眼和嘴形必须继承原画的线稿、睫毛体量、眼角、口腔配色、阴影和抗锯齿，不能用一条弧线或粗糙符号糊弄；完整规则见 `references/visual-rigging-rules.md`。
