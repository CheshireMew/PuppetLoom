...
如果错误只是几个完整独立图层的前后顺序，候选可标为 `accepted-with-repairs`，`repairPlan` 逐条记录要移动的 layer ID、相对目标和视觉原因；进入项目后由 Agent 通过可恢复的 `move-layer` authoring 操作调整，再查看 neutral、眨眼和身体动作证据。若前后裙、前后发、眉毛与脸或其它前后结构已经粘在同一图层，无法只靠移动完整图层修复，则标为 `rejected`。缺失核心五官或主要身体区域、第二主体串层、Alpha 合成后存在大面积白雾/底色/绿边、有效 Alpha 内有影响主体的污染、画布严重错位或分层结构不足以可靠创建项目时同样拒绝；缺少呆毛、小配饰、少量边缘残留、孤立噪点和轻微颜色变化等不改变主要轮廓与遮挡的问题才可后修。写完结论后通过确定性入口校验三档状态并同步结果，Agent 不再手工维护两份状态：
