...
用户要求增强制作能力或借鉴其它项目时，先按目标部位比较同一素材的实际效果与实现，再选择要改的地方。脸部看中立、左右、上下和斜向组合，分清整体体积、五官修形及遮挡；头发先分开检查随头转动与惯性摆动。记录参考的具体方法、当前缺陷、采用或不采用的理由，并用同素材前后画面验证。参数名字和范围相同不代表角度或运动相同，须注明方向、幅度和渲染入口。测试与工具改进不能代替制作效果，某一部位完成后继续处理原任务剩余部位，不把整模任务收窄成反复修一个局部。
...
- 单个关键形的局部轮廓偏差：按 `asset-and-geometry-workflow.md` 使用分页 `author geometry` 与区域 `transform-keyform`；轴心或权重偏差继续用稀疏 `calibrate`，保留 from/to 证据。
- 已授权的拆发、头饰或其它素材替换：按 `asset-and-geometry-workflow.md` 准备参考、配准并尽早试装，先区分摆放、遮挡、绑定和素材绘制问题，再决定重画。
- 先判断闭眼是否真的缺素材：左右眼白与睫毛完整时，眼部制作入口可从原图生成几何闭合关键形，以眼白遮罩隐藏虹膜，不必补画闭眼图。先看半闭、全闭和转头组合中的睫毛形状、厚度与眼角，再决定是否采用；有自定义眨眼绑定时保留并检查父级，不能叠加两套闭合。需要特定手绘闭眼造型时可使用贴图模式，切换时保留关键形。张口等确实缺少的原画素材，经验证后通过 `enhance` 接入；已有闭嘴不重画。微张嘴或音素嘴形只有用户明确需要时才增加。不能以“缺闭眼贴图”直接判定角色不能眨眼，也不能以参数有变化代替画面检查。
- 结构和安全检查全绿但画面难看、僵硬或破坏原效果：仍判为 `repair` 或失败；这是外部 Agent 必须承担的视觉判断。

## 给用户的验收

`agent apply` 成功执行后返回 `status: awaiting-visual-review`、`visualReview: unreviewed`；`ok: true` 只代表本次制作命令执行成功。分部报告的 `executionStatus: succeeded` 同样不代表造型合格。外部 Agent 必须根据准确 revision 的原画、局部阶段图、整体图和连续动作作出视觉判断；无需新增内置模型或聊天服务。报告一旦指出具体造型缺陷，该项就保持未通过，修复并重新看图后才能改变结论。历史制作报告保留执行时状态，用户接受后的状态以对应校准 session 为准，不把 Agent 自检冒充用户接受。

先展示候选角色窗口或准确 revision 的前后/连续效果，再报告完成部位、缺失部位、素材边界和仍需用户判断的观感。每个未通过项说明它在什么姿态或帧出现、画面具体哪里不对、证据在哪里以及当前能否修复；不要只报告检查计数。候选接受后才把对应 session 标为 `accepted` 并运行完整回归；拒绝后标记受影响 session 和具体缺陷；撤回只针对相关变化。用户已授权继续返修或给出新方向时，修正目标再预览，不自动停止整个任务。不要把测试清单当作主要交付。用户不满意时先接收“哪里看起来不对”的结果反馈，由 Agent 自己定位部位和参数；只有 CLI 不能表达的局部问题才邀请用户手调。

闭眼或头脸需要实际修形时使用 `shape-target-workflow.md` 的结构标记、目标点/曲线及只读预览。先核对像素职责，再看目标和实际输出；不能把默认生成器当作针对人物的造型判断。
