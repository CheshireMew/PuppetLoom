...
パペットウィンドウは最前面、マウス透過、システムマウス追従、ローカル顔トラッキング、マイク口パク、実在する表情・アクションの実行、録画に対応します。録画では透明・単色背景、解像度、24/30/60 FPS、手動または時間指定停止、マイク音声を選べます。完成 WebM には再生可能な入力 JSON が付き、中断時は partial ファイルと報告を残します。
