...
`enhance` 只接受与请求尺寸完全相同、Alpha 与覆盖率通过的 PNG。两只闭眼都通过后才启用自动眨眼；嘴形素材分别标记为 `closed`、`slight` 和 `open`。新的 `closed` 通过后才把 PSD 原嘴层透明度置零，因此失败时仍有原嘴回退。没有驱动时显示新的自然闭口层。失败文件只写入拒绝结果，不修改已经安全的项目。
