...
export function EditorLayerPanel({
  ...
  const ordered = [...project.layers].sort((left, right) => right.order - left.order);
  return (
        ...
        {ordered.map((layer) => (
          <div key={layer.id} className={`layer-row ${selectedLayerId === layer.id ? "selected" : ""} ${layer.visible === false ? "hidden" : ""}`} style={{ paddingLeft: `${8 + layerDepth(layer, byId) * 16}px` }}>
            <button className="layer-select" onClick={() => onSelect(layer.id)}>
              <span>{layer.sourceName}</span><small>{layer.role} · {layer.side} · #{layer.order}</small>
            </button>
            <button className="layer-icon" title={layer.visible === false ? "显示图层" : "隐藏图层"} aria-label={`${layer.sourceName} ${layer.visible === false ? "显示" : "隐藏"}`} onClick={() => onPatchLayer(layer.id, { visible: layer.visible === false })}>{layer.visible === false ? "○" : "●"}</button>
            <button className="layer-icon" title={layer.locked ? "解锁图层" : "锁定图层"} aria-label={`${layer.sourceName} ${layer.locked ? "解锁" : "锁定"}`} onClick={() => onPatchLayer(layer.id, { locked: !layer.locked })}>{layer.locked ? "锁" : "开"}</button>
          </div>
        ))}
      </div>
    </aside>
  ...
  return (
    <section className="viewport-panel">
      <div
        ...
        className={`editor-viewport ${navigation.panning ? "is-panning" : ""} ${navigation.spacePressed ? "is-space-ready" : ""}`}
      ...
      >
        ...
        >
          ...
          <svg className="editor-overlay" viewBox="0 0 1 1" preserveAspectRatio="none" onPointerMove={onMoveDrag} onPointerUp={onEndDrag} onPointerCancel={onEndDrag}>
          {mode === "semantic" && cage && <>
          ...
          {mode === "mesh" && selectedLayer && <>
            ...
            {meshPoints.map((point, index) => <circle key={index} cx={point.x} cy={point.y} r={selectedVertex === index ? "0.007" : "0.0045"} className={`handle mesh-handle ${selectedVertex === index ? "selected" : ""} ${locked ? "locked" : ""}`} tabIndex={locked ? -1 : 0} role="slider" aria-label={`${selectedLayer.sourceName} 网格顶点 ${index}`} aria-valuetext={`${point.x.toFixed(3)}, ${point.y.toFixed(3)}`} onFocus={() => onSelectVertex(index)} onKeyDown={(event) => onNudge(event, { kind: "mesh", index })} onPointerDown={(event) => onBeginDrag(event, { kind: "mesh", index })} />)}
          </>}
          </svg>
        </div>
      ...
      <p className="viewport-help">滚轮会以鼠标位置为中心缩放；拖动空白处、按住空格拖动或使用鼠标中键可移动视图；双击空白处恢复适配。拖动控制点仍会直接校准。</p>
    ...
    <aside className="inspector-panel">
      ...
      {selectedLayer ? <>
        ...
        <section className="mesh-density">
          ...
          <small>改变密度会重建网格、重新采样权重，并退出旧顶点编辑。</small>
        </section>
